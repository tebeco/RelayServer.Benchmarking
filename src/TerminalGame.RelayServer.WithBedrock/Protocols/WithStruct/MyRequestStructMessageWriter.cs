...
{
    public class MyRequestStructMessageWriter : IMessageWriter<IStructMessage>
    {
        public void WriteMessage(IStructMessage message, IBufferWriter<byte> stream)
        {
            WriteHeaders(message, stream);
            WriteContent(message, stream);
        }

        private static void WriteHeaders(IStructMessage message, IBufferWriter<byte> stream)
        {
        ...
        private static void WriteContent(IStructMessage message, IBufferWriter<byte> stream)
        {
            ...
            try
            {
                ...
                switch (message)
                {
                    case InitStructMessage m:
                        WriteInitMessage(m, writer);
                        break;
                    case PayloadStructMessage m:
        ...
        private static void WriteInitMessage(InitStructMessage message, Utf8JsonWriter writer)
        {
            WritePayloadType(message, writer);
            WriteSource(message, writer);
        }

        private static void WritePayLoadMessage(PayloadStructMessage message, Utf8JsonWriter writer)
        {
            WritePayloadType(message, writer);
            WriteSource(message, writer);
        ...
        private static void WritePayloadType(IStructMessage message, Utf8JsonWriter writer)
        {
        ...
        private static void WriteSource(IStructMessage message, Utf8JsonWriter writer)
        {
        ...
        private static void WriteDestination(PayloadStructMessage message, Utf8JsonWriter writer)
        {
        ...
        private static void WritePayload(PayloadStructMessage message, Utf8JsonWriter writer)
        {
